...
def register_success(): #註冊成功的介面
    reg_success_window = tk.Toplevel(register_window)
...
def register_window(): #輸入註冊資料
    ...
    register_window=tk.Toplevel(window1)
    register_window.title('註冊')
    register_window.geometry('300x300')

    login_label=tk.Label(register_window,text="請輸入用戶名稱、密碼和Line Token：")
    login_label.pack()
    
    username_label=tk.Label(register_window,text="用戶名稱：")
    username_label.pack()

    username_entry=tk.Entry(register_window)
    username_entry.pack()

    password_label=tk.Label(register_window,text='密碼')
    password_label.pack()
    
    password_entry=tk.Entry(register_window)
    password_entry.pack()

    token_label=tk.Label(register_window,text="Token:")
    token_label.pack()

    token_entry=tk.Entry(register_window)
    token_entry.pack()
    ...
    def register():
        ...
        if register_user(username,password,token):
            register_window.destory()
        
    register_button=tk.Button(register_window,text="註冊",command=register)
...
login_button.pack()
register_button=tk.Button(window1,text='註冊',command=register_window)
register_button.pack()
